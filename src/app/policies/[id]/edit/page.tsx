...
import { GlobalPolicy } from '@/lib/types'
...
export default function EditPolicyPage({ params }: { params: { id: string } }) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setFormData({
            ...
            year: policyData.year || new Date().getFullYear(),
            period: policyData.period || 'H1',
            effective_date: policyData.effective_date || '',
            description: policyData.description || '',
            
            pension_base_floor: policyData.pension_base_floor || 1900,
            pension_base_cap: policyData.pension_base_cap || 24330,
            pension_rate_individual: policyData.pension_rate_individual || 0.08,
            pension_rate_company: policyData.pension_rate_company || 0.14,
            
            medical_base_floor: policyData.medical_base_floor || 1900,
            medical_base_cap: policyData.medical_base_cap || 24330,
            medical_rate_individual: policyData.medical_rate_individual || 0.02,
            medical_rate_company: policyData.medical_rate_company || 0.055,
            
            unemployment_base_floor: policyData.unemployment_base_floor || 1900,
            unemployment_base_cap: policyData.unemployment_base_cap || 24330,
            unemployment_rate_individual: policyData.unemployment_rate_individual || 0.0032,
            unemployment_rate_company: policyData.unemployment_rate_company || 0.008,
            
            injury_base_floor: policyData.injury_base_floor || 1900,
            injury_base_cap: policyData.injury_base_cap || 24330,
            injury_rate_company: policyData.injury_rate_company || 0.001,
            
            hf_base_floor: policyData.hf_base_floor || 1900,
            hf_base_cap: policyData.hf_base_cap || 34860,
            hf_rate_individual: policyData.hf_rate_individual || 0.05,
            hf_rate_company: policyData.hf_rate_company || 0.05,
            
            is_active: policyData.is_active !== false,
            created_by: policyData.created_by || 'system'
          })
        } else {
          alert(`加载政策失败: ${result.error}`)
  ...
  const handleDelete = async () => {
    ...
    if (!confirm(`确定要删除政策"${policy.policy_name}"吗？此操作不可撤销。`)) {
      return
    }
  ...
  if (!policy) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <div className="text-red-500">政策不存在</div>
          <Link href="/policies" className="mt-4 inline-block">
            <Button>返回政策列表</Button>
          </Link>
        </div>
      </div>
    )
      ...
      <div className="px-4 py-6 sm:px-0">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">编辑政策</h1>
              <p className="text-gray-600 mt-2">修改五险一金政策规则</p>
            </div>
          </div>
